...
DxSubscription::DxSubscription(JNIEnv *env, jobject dxSubscription) : subscription_(env->NewGlobalRef(dxSubscription)) {
}
...
DxSubscription *DxSubscription::create(JNIEnv *env, jobject conn, dxfg_event_clazz_list_t *eventClasses,
    ...
    javaLogger->trace(env, "create(jobject, dxfg_event_clazz_list_t*, bool), jDxSubscription: %", jDxSubscription);
...
dxfg_event_clazz_list_t *DxSubscription::getEventTypes(JNIEnv *env) {
    ...
    auto jByteArray =
        checkedCallStaticObjectMethodAndCastTo<jbyteArray>(env, jDxSubscriptionJniClass, methodId, subscription_);
